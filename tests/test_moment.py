...
class TestMoment(unittest.TestCase):
    ...
    def test_ts_to_dt(self):
        ...
        self.assertEqual(
            value_dt_utc.strftime("%Y-%m-%d %H:%M:%S %Z"), "2015-03-14 00:00:00 UTC"
        )
        self.assertEqual(
    ...
    def test_dst_switches(self):
        ...
        self.assertEqual(
            ts_to_dt_utc(dst_before).strftime(fmt), "1918-03-31 06:59:59 UTC"
        )
        self.assertEqual(
            ts_to_dt_utc(dst_begin).strftime(fmt), "1918-03-31 07:00:00 UTC"
        )
        self.assertEqual(ts_to_dt_utc(dst_end).strftime(fmt), "1918-10-27 05:59:59 UTC")
        self.assertEqual(
            ts_to_dt_utc(dst_after).strftime(fmt), "1918-10-27 06:00:00 UTC"
        )
        ...
        self.assertEqual(
            ts_to_dt_nyc(dst_before).strftime(fmt), "1918-03-31 01:59:59 EST"
        )
        self.assertEqual(
            ts_to_dt_nyc(dst_begin).strftime(fmt), "1918-03-31 03:00:00 EDT"
        )
        self.assertEqual(ts_to_dt_nyc(dst_end).strftime(fmt), "1918-10-27 01:59:59 EDT")
        self.assertEqual(
            ts_to_dt_nyc(dst_after).strftime(fmt), "1918-10-27 01:00:00 EST"
        )
        self.assertEqual(
            ts_to_dt_nyc(dst_after + 3599).strftime(fmt), "1918-10-27 01:59:59 EST"
        )
    ...
    def test_tzinfo(self):
        ...
        self.assertEqual(
            (nyc_dt1 + timedelta(seconds=3601)).strftime(fmt), "1979-04-29 03:00:00 EDT"
        )
        self.assertEqual(
    ...
    def test_dt_to_ds(self):
        ...
        self.assertEqual(
            value_dt.strftime("%Y-%m-%d %H:%M:%S %Z"), "2015-03-14 00:00:00 "
        )
        ...
        self.assertEqual(
            format_utc(moment.dt_to_ts(value_dt, tzla)), "2015-03-14 07:00:00 UTC"
        )
        # The second argument is ignored if the datetime is aware.
        self.assertEqual(
            format_utc(moment.dt_to_ts(value_dt_utc, tzla)), "2015-03-14 00:00:00 UTC"
        )
        self.assertEqual(
            format_utc(moment.dt_to_ts(value_dt_aware, tzla)), "2015-03-14 00:00:00 UTC"
        )
    ...
    def test_date_to_ts(self):
        ...
        self.assertEqual(
            format_utc(moment.date_to_ts(d, tzla)), "2015-03-14 07:00:00 UTC"
        )
        self.assertEqual(
