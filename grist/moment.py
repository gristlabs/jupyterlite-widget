...
def ts_to_dt(timestamp, zone, tzinfo=None):
    return (EPOCH_UTC + timedelta(seconds=timestamp)).astimezone(
        tzinfo or zone.get_tzinfo(None)
    )
...
class TzInfo(_tzinfo):
    ...
    def tzname(self, dt):
        ...
        return self.zone.dt_tzname(dt, self._favor_offset)
...
class Zone(object):
    ...
    def __init__(self, zonelabel):
        ...
        self.offset_untils = [
            until - offset * 60000 for (until, offset) in zip(self.untils, self.offsets)
        ]
    ...
    def _index_dt(self, dt, favor_offset):
        ...
        if (
            i < len(self.offset_untils)
            and timestamp >= self.untils[i] - self.offsets[i + 1] * 60000
        ):
    ...
    def get_tzinfo(self, favor_offset):
        ...
        """
        return self._tzinfo.get(favor_offset) or self._tzinfo.setdefault(
            favor_offset, TzInfo(self, favor_offset)
        )
...
def get_zone(zonelabel):
    ...
    return _zone_cache.get(zonelabel) or _zone_cache.setdefault(
